...
class fb{
	...
	public function createLoginLink()
	{
		...
		'scope' => 'email',
		'display' => 'popup',
		'redirect_uri' => 'http://lovechannel.com/home/fblogin',
		);
		return $url = $this->CI->facebook->getLoginUrl($login_params); 
	}
	
	public function createLoginLinkChallenge($challenge_site,$usertype,$featured_id)
	{
		$login_params = array(
		'scope' => 'email',
		'display' => 'popup',
		'redirect_uri' => base_url().'signup/fblogin/challenger?domain='.$challenge_site.'&featured_challenge_id='.$featured_id,
		);
		return $url = $this->CI->facebook->getLoginUrl($login_params); 
	}
